...
export default function RecuperarEvaluacion() {
    ...
    useEffect(() => {
        const fetchEstudiantes = async () => {
            try {
                const response = await axios.get('http://innovalogic.tis.cs.umss.edu.bo/CargarEvaluaciones.php', {
    ...
    const cargarDatosGrupo = async (idDocente) => {
        try {
            const response = await axios.get('http://innovalogic.tis.cs.umss.edu.bo/CargarGrupoDocente.php', {
                params: { idDocente }
            });
    ...
    return (
        <>
            ...
            <div style={{ display: 'flex', height: 'calc(-110px + 100vh)', marginTop: '70px', backgroundColor: '#32569A' }}>
                <BarraLateralDocente/>
                <form className={`space-y-4 p-4 flex-1 bg-[#efe7dc] rounded-md`} style={{  overflowY: 'auto'  }}>
