...
export default function RegistroEvSemanales() {
    ...
    useEffect(() => {
        ...
        if (selectedGrupo) {
            fetch(`http://innovalogic.tis.cs.umss.edu.bo/obtenergruposevsemsprints.php?idGrupo=${selectedGrupo}`) // Cambia la ruta según tu estructura
                .then(response => {
                    if (!response.ok) {
    ...
    useEffect(() => {
        const fetchEstudiantesConEvaluaciones = async () => {
            if (selectedGrupo && selectedSprint && selectedSemana) {
                ...
                try {
                    const response = await fetch(`http://innovalogic.tis.cs.umss.edu.bo/buscarEstudiantesdelgrupo.php?grupo=${selectedGrupo}&sprint=${selectedSprint}&semana=${selectedSemana}`);
                    ...
                    const estudiantesConEvaluaciones = await Promise.all(data.map(async estudiante => {
                        ...
                        const response = await fetch(`http://innovalogic.tis.cs.umss.edu.bo/buscarEvaluacionAnterior.php?idEstudiante=${estudiante.idEstudiante}&semana=${selectedSemana ? semanas.find(semana => semana.inicio === selectedSemana)?.nombre:''}`);
    ...
    const handleGuardarEvaluaciones = () => {
        ...
        fetch('http://innovalogic.tis.cs.umss.edu.bo/guardarEvaluaciones.php', {
            method: 'POST',
            headers: {
