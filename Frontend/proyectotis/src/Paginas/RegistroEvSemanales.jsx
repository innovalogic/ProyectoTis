...
export default function RegistroEvSemanales() {
    ...
    useEffect(() => {
        ...
        if (selectedGrupo) {
            fetch(`https://tis-e8f3f498eaee.herokuapp.com/obtenergruposevsemsprints.php?idGrupo=${selectedGrupo}`) // Cambia la ruta según tu estructura
                .then(response => {
                    if (!response.ok) {
    ...
    useEffect(() => {
        const fetchEstudiantesConEvaluaciones = async () => {
            if (selectedGrupo && selectedSprint && selectedSemana) {
                ...
                try {
                    const response = await fetch(`https://tis-e8f3f498eaee.herokuapp.com/buscarEstudiantesdelgrupo.php?grupo=${selectedGrupo}&sprint=${selectedSprint}&semana=${selectedSemana}`);
                    ...
                    const estudiantesConEvaluaciones = await Promise.all(data.map(async estudiante => {
                        ...
                        const response = await fetch(`https://tis-e8f3f498eaee.herokuapp.com/buscarEvaluacionAnterior.php?idEstudiante=${estudiante.idEstudiante}&semana=${selectedSemana ? semanas.find(semana => semana.inicio === selectedSemana)?.nombre:''}`);
    ...
    const handleGuardarEvaluaciones = () => {
        ...
        fetch('https://tis-e8f3f498eaee.herokuapp.com/guardarEvaluaciones.php', {
            method: 'POST',
            headers: {
